...
class VCOCOeval(object):
  ...
  def _prep_vcocodb_entry(self, entry):
    ...
    if float(np.__version__[:4]) >= 1.24:
  ...
  def _do_role_eval(self, vcocodb, detections_file, ovr_thresh=0.5, eval_type='scenario_1'):
    ...
    print('---------------------------------------------')
    with open("vcoco_results_official.txt", 'a') as f:
      f.write(json.dumps('Average Role [%s] AP = %.2f' % (eval_type, np.nanmean(role_ap) * 100.00)) + "\n")
      f.write(json.dumps('Average Role [%s] AP = %.2f, omitting the action "point"' % (
                        eval_type, (np.nanmean(role_ap) * 25 - role_ap[-3][0]) / 24 * 100.00)) + "\n")
      ...
      for aid in range(self.num_actions):
        ...
        if float(np.__version__[:4]) >= 1.24:
          covered = np.zeros((gt_boxes.shape[0]),
                             dtype=np.bool_)  # gt_boxes.shape[0] is the number of people in this image
        else:
          covered = np.zeros((gt_boxes.shape[0]),
