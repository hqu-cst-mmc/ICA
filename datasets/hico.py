...
class HICODetection(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 重新设置注解中关于框的排列大小,确保一行有四个值为框的四个坐标
        ...
        target = {}
        target['orig_size'] = torch.as_tensor([int(h), int(w)])    # 'orig_size'和 'size' 两个key ，存储原图片高、宽
        target['size'] = torch.as_tensor([int(h), int(w)])
        if self.img_set == 'train':
            boxes[:, 0::2].clamp_(min=0, max=w)
            boxes[:, 1::2].clamp_(min=0, max=h)
            keep = (boxes[:, 3] > boxes[:, 1]) & (boxes[:, 2] > boxes[:, 0])    # keep 表示有效的框，即左上小于右下这样，过滤掉无效的那些框
            boxes = boxes[keep]
...
def make_hico_transforms(image_set):
    ...
    normalize = T.Compose([
        T.ToTensor(),   # 把图像的通道维放在最前面，并对像素归一化[0,1]
        T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])   # 根据指定的均值和标准差对图像进行归一化，同时将标签的bbox转化成cx,cy,w,h的形式后归一化到[0,1]
    ])
...
def build(image_set, args):
    ...
    # 根据数据集类型，获得不同的数据集文件路径和注解路径
    img_folder, anno_file = PATHS[image_set]
    # 构造数据集
    dataset = HICODetection(image_set, img_folder, anno_file, transforms=make_hico_transforms(image_set),
                            num_queries=args.num_queries)
    if image_set == 'val':
        dataset.set_rare_hois(PATHS['train'][1])    # 当构建验证数据集时，才区分rare和no-rare
        dataset.load_correct_mat(CORRECT_MAT_PATH)  # 加载corre_hico.npy文件,获得数据的[N,H,W,C]，N为数据集总数，H、W、C分别对应高、宽、通道数
    return dataset
