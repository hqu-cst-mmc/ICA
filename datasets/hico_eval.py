        ...
        if self.eval_mode == 'default':
            ...
            for img_preds, img_gts in zip(preds, gts):
                ...
                if len(subject_ids) > 0:
                    ...
                    hois.sort(key=lambda k: (k.get('score', 0)), reverse=True)
                    hois = hois[:self.max_hois]
                    hois = [{'subject_id': hoi['subject_id'], 'object_id': hoi['object_id'], 'category_id': hoi['category_id'], 'score': hoi['score']}
                            for hoi in hois if bboxes[hoi['object_id']]['category_id'] in gt_obj_category_id]
                else:
                    hois = []
